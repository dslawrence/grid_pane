...
"_off"!=b.vertical_center||"_off"!=b.vertical_center)(d=b.target.html())||(d="9"),e=b.target.height(),b.target.width(),m.html(b.target.clone()),m.find(":first").html('<div class="the_aligner" style="position:relative;">'+d+'<div style="clear:both;"></div></div>'),m.show(),d=g(".the_aligner").height(),m.hide(),d=Math.floor((e-d)/2),0<d&&b.target.css({"padding-top":d,height:e-d});c.meta.post_render(c)};m.html("");window.grid_panes.push(c);c.run();return c}})(jQuery);
$(function(){setInterval(function(){window.grid_pane_run_needed&&(run_all_grid_panes(),window.grid_pane_run_needed=0)},100);$(window).bind("orientationchange resize",function(){window.grid_pane_run_needed=1})});function run_all_grid_panes(){for(var g=0;g<window.grid_panes.length;g++)window.grid_panes[g].run()};
